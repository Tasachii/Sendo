...
export async function GET(req: Request) {
  ...
  const lines = rows.map((r) =>
    [r.monthLabel, r.count, b(r.subtotalSatang), b(r.vatSatang), b(r.whtSatang), b(r.netSatang)].join(",")
  );
  const total = rows.reduce(
    (a, r) => ({ c: a.c + r.count, s: a.s + r.subtotalSatang, v: a.v + r.vatSatang, w: a.w + r.whtSatang, n: a.n + r.netSatang }),
    { c: 0, s: 0, v: 0, w: 0, n: 0 }
  );
  lines.push(["รวม", total.c, b(total.s), b(total.v), b(total.w), b(total.n)].join(","));
